...
public class StudentQueriesSolution {
	public static void main(String[] args) {
		...
		List<Student> students = Arrays.asList(
				new Student(101, "Sachin", 22, "Male", "Mumbai", 12, "Computer",
				...
				new Student(104, "Neha", 21, "Female", "Mumbai", 132, "Electronics",
				...
				new Student(107, "Karan", 26, "Male", "Bangalore", 19, "Electronics",
		...
		System.out.println("\n3. Only names:");
		students.stream().map(Student::getFirstName).forEach(System.out::println);

		System.out.println("\n4. Mechanical Engineering students:");
		students.stream().filter(s -> "Mechanical".equals(s.getDepartment())).forEach(System.out::println);

		System.out.println("\n5. Not from Mumbai:");
		students.stream().filter(s -> !"Mumbai".equals(s.getCity())).forEach(System.out::println);
		...
		students.stream().sorted(Comparator.comparing(Student::getFirstName).thenComparingInt(Student::getRank))
				.forEach(System.out::println);
		...
		System.out.println("\n3. Second best rank:");
		students.stream().sorted(Comparator.comparingInt(Student::getRank)).skip(1).findFirst()
		...
		System.out.println(
				"\n2. Total contact numbers: " + students.stream().flatMap(s -> s.getContacts().stream()).count());
		...
		System.out.println("\n======= Conditional & Matching =======");
		System.out.println(
				"1. Any Biotech student? " + students.stream().anyMatch(s -> "Biotech".equals(s.getDepartment())));

		System.out.println("2. All students > 20 years? " + students.stream().allMatch(s -> s.getAge() > 20));
		...
		System.out.println("2. Avg age - Male: " + students.stream().filter(s -> "Male".equals(s.getGender()))
				.mapToInt(Student::getAge).average().orElse(0));

		System.out.println("3. Avg age - Female: " + students.stream().filter(s -> "Female".equals(s.getGender()))
				.mapToInt(Student::getAge).average().orElse(0));

		System.out.println("4. Highest ranked Male:");
		students.stream().filter(s -> "Male".equals(s.getGender())).min(Comparator.comparingInt(Student::getRank))
				.ifPresent(System.out::println);

		System.out.println("5. Highest ranked Female:");
		students.stream().filter(s -> "Female".equals(s.getGender())).min(Comparator.comparingInt(Student::getRank))
				.ifPresent(System.out::println);
		...
		students.stream().filter(s -> "Computer".equals(s.getDepartment()))
				.min(Comparator.comparingInt(Student::getAge)).ifPresent(System.out::println);

		System.out.println("\n3. Top ranked in Mechanical:");
		students.stream().filter(s -> "Mechanical".equals(s.getDepartment()))
				.min(Comparator.comparingInt(Student::getRank)).ifPresent(System.out::println);

		System.out.println("\n4. Electronics students from Karnataka:");
		long count = students.stream().filter(s -> "Electronics".equals(s.getDepartment()))
				.filter(s -> "Karnataka".equals(s.getCity()) || "Bangalore".equals(s.getCity())).count();
		...
		students.stream().filter(s -> s.getFirstName().startsWith("S")).map(Student::getFirstName)
				.forEach(System.out::println);

		System.out.println("\n4. Name length > 5:");
		students.stream().filter(s -> s.getFirstName().length() > 5).map(Student::getFirstName)
				.forEach(System.out::println);

		System.out.println("\n5. Even rank:");
		students.stream().filter(s -> s.getRank() % 2 == 0).map(Student::getFirstName).forEach(System.out::println);

		System.out.println("\n6. Odd rank:");
		students.stream().filter(s -> s.getRank() % 2 == 1).map(Student::getFirstName).forEach(System.out::println);
		...
		countByCity.entrySet().stream().max(Map.Entry.comparingByValue())
				.ifPresent(e -> System.out.println(e.getKey() + " → " + e.getValue()));

		System.out.println("\n6. Department with minimum students:");
		countByDept.entrySet().stream().min(Map.Entry.comparingByValue())
				.ifPresent(e -> System.out.println(e.getKey() + " → " + e.getValue()));
	}
}